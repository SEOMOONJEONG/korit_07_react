...
```
```
async/await는 “Promise를 더 읽기 쉽게 만든 문법”으로,
await은 “결과가 올 때까지 잠시 기다렸다가 다음 줄로 넘어간다”는 뜻
```
...
📞 “서버에 전화 걸기(fetch)” 
☕ “응답 기다리기(await)” 
📄 “받은 답변을 읽기 좋게 정리(response.json())” 
✅ “정상적으로 끝나면 다음 일 실행” 
❌ “전화 연결이 안 되면(catch) → 오류 출력” 

try { await ... } catch (error) 구조는, “비동기 코드가 실패해도 앱이 멈추지 않게 예외를 잡는 안전장치"
```
...
## fetch API 이용
JS 상에서의 fetch API 개념은 기존의 XMLHttpRequest 혹은 jQuery Ajax API와 유사한데, fetch API가 좋은 점은 Promise를 지원한다는 점도 장점입니다.

fetch API는 아까 위에서 봣떤 fetch() 메서드를 제공합니다. 웹 요청의 경우에 argument는 서비스의 URL이 됩니다. 그래서 저희는 처음으로 `GET 요청`을 react에서 하는 예시를 작성하게 될겁니다.
```jsx
...
이상의 의사 코드에서 `.then(response => response.json())` 라인의 then 문에 전달되는 response는 요청이 성공했는지 확인하는 데 이용하는 ok 및 status 속성을 포함하는 객체입니다. 응답 상태가 2xx 형식이라면 ok 값 속성은 true로 return됩니다.
...
```
POST와 같은 다른 HTTP 메서드를 이용하려면 fetch() 메서드의 두 번째 argument에 대항 메서드를 정의해야 합니다. 두 번째 argument는 여러 요청 설정을 정의할 수 있는 _객체_ 입니다. 그래서 이하의 예시는 POST 요청에 대한 부분입니다.
...
.catch(error => console.log(error));
```
근데 저희가 postman에서 해봤던 것처럼 특정한 자격이 있거나 특정 형식으로 headers를 추가할 필요가 있었습니다. 그럼 fetch()의 두 번재 argument인 객체에 key-value properties를 추가함으로써 해결할 수 있습니다.
...
- 여기가 특정 라이브러리를 사용하게 될 경우 우리는 추가 설치를 해야할 경우가 있습니다.
```
npm install axios

github에 올라가지 않음.
```
axios 라이브러리는 JSON 데이터의 자동 변환과 같은 이점을 지니고 있습니다. 그래서 fetch()에서 response.json() 메서드를 호출했던 것과 같은 과정이 필요 없습니다.
```jsx
// GET
...
```jsx
// POST
axios.get('http://someapi.com', { 새로운 객체 })
...
회원가입 후 로그인 → MyAPI → key값 확인

REST API를 이용하는 경우 응답을 검사하여 JSON 데이터의 포맷을 확인해야 합니다. 즉 날씨 정보를 OpenWeather API로 활용할 겁니다.

https://api.openweathermap.org/data/2.5/weather?q=Busan&units=Metric&APIkey=e892b3619e3183f352b952e0947340db


https://openweathermap.org/img/wn/04d@2x.png
여기서 04d가 icon key의 value로 잡혀있고, img 파일을 불러오기 위해서는 02x.png을 붙여줘야 합니다.
...
2. useEffect()의 argument 분석입니다.
  ...
  - callback function의 내부 로직은 fetchAPI를 활용했었습니다.(axios가 아니라)
...
그리고 md파일 부분에서
git add .
git commit -m "feat: 20251017 Promise / async / await"
git push
...
.catch(arror => conosole.log(error));
```
...
`q=` 는 query를 뜻합니다. = 다음에 우리가 검색하는 내용의 키워드를 집어넣게 되면 그 결과값을 GET 요청으로 가져오게 됩니다.
즉, 우리는 input 창 입력한 내용을 {KEYWORD}에 대입하게 될겁니다.

이상의 결과값에 들어가게 되면, JSON 데이터가 나오는데, 그 중 items 키를 확인하면 배열로 이루어져 있다는 것을 알 수 있습니다.
...
즉, input창에 값을 입력하면, 그 값을 가지고 api에 GET 요청을 날리게 되고, 그 결과값을 JSON 응답데이터 상태에 저장하게 되는 순서(weather 상태를 저장한 것처럼)입니다.
...
github API가 요청 횟수 제한이 있습니다. 현재 저희가 별 문제가 없긴 한데, 대충 1 분에 10회 정도로 알고 있습니다. 이 한도를 넘어가면 또 1 분 기다린 후에 테스트 할 수 있습니다.
...
npm install axios
npm install
npm run dev 해서 default page 켜세요
App.jsx 초기화해서 빈 브라우저를 켜세요.

git add .
git commit -m "feat:20251017 gitapi app created
...
(위 코드와 아래코드의 차이는 객체 구조 분해의 차이. 배열이 아니라 중괄호로 작성.)
```jsx
...
이 출력된 리포지토리들은 리액트 쿼리 라이브러리를 이용하여 가져왔습니다. 또한 내장도니 기능을 이용하여 요청 상태를 isLoading / isError / data의 형태로 처리하였으며, 리액트 쿼리가 데이터 관리와 캐싱을 처리하기 때문에 응답 데이터를 추가로 저장하기 위한 상태의 선언이 필요하지 않습니다.

근데 그 chrome에서 탭을 왔다갔다 하다보면 브라우저가 재포커스가 되는데, 리액트 쿼리에 의해서 다시 불러오기가 실행됩니다.(근데 q=korit_07의 검색 결과가 너무 적어서 빠르게 로딩이 일어납니다.) 그리고 이거랑 아까 말한 github api 자체 정책으로 인해 1 분 에 10 번 이상 GET 요청을 날려버려서 오류가 발생합니다.
